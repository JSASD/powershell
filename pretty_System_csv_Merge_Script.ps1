...
$Logs = "\\js-vessel\Technology\Logging\Server-Logs\System-Logs"

# Specify the path to the destination folder
$Master = "\\js-vessel\Technology\Logging\Server-Logs\System-Logs\Master"

# Get a list of CSV files in the logs folder, excluding the current file
$CSV = Get-ChildItem $Logs\*.csv -Exclude "$Logs\$fileName System_Logs.csv"
...
$Merged | Set-Content "$Master\$fileName System_Logs.csv"

# Remove all CSV files in the logs folder except the newly created file
$CSV | Where-Object {$_.Name -ne "$fileName System_Logs.csv"} | Remove-Item -Force